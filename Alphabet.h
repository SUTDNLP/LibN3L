...
public:
  ...
  void read(const std::string& inputModelFile)
  {
    clear();
    static string tmp;
    ifstream inf;
    inf.open(inputModelFile.c_str());
    while(1) {
      my_getline(inf, tmp);
      if (tmp == "BEGIN_ALPHABET:") {
        cout << "Begin read alphabet..." << endl;
        break;
      }      
    }
    my_getline(inf, tmp);
    ...
    for (int i = 0; i < m_size; ++i) {
      my_getline(inf, tmp);
      if (tmp == "END_ALPHABET!") {
        cout << "Finish read alphabet!" << endl;
        break;
      }
      split_bychars(tmp, featids);
      m_string_to_id[featids[0]] = i;
  ...
  void write(const std::string& outputModelFile) const
  { ofstream outf;
    outf.open(outputModelFile.c_str(), ios_base::app);
    outf << "BEGIN_ALPHABET:" << std::endl;
    outf << m_size << std::endl;
    for (int i=0; i<m_size; i++)
    {
      outf << m_id_to_string[i] << " " << i << std::endl;
    }
    outf << "END_ALPHABET!" << std::endl;
    outf << std::endl;
  }
};
