...
#ifndef SRC_LSTM_KER_H_
#define SRC_LSTM_KER_H_
#include "tensor.h"
...
class LSTM_KER {
public:
  BiLayer<xpu> _lstm_output;
  BiLayer<xpu> _lstm_input;
  BiLayer<xpu> _lstm_forget;
...
public:
  ...
  inline void ComputeForwardScore(Tensor<xpu, 3, dtype> x, Tensor<xpu, 3, dtype> iy, Tensor<xpu, 3, dtype> oy, Tensor<xpu, 3, dtype> fy,
    ...
    if (_left2right) {
      for (int idx = 0; idx < seq_size; idx++) {
        if (idx == 0) {
          _lstm_input.ComputeForwardScore(_null1, x[idx],  iy[idx]);
          _lstm_cell.ComputeForwardScore(_null1, x[idx], mcy[idx]);
          cy[idx] = mcy[idx] * iy[idx];
          _lstm_output.ComputeForwardScore(_null1, x[idx], oy[idx]);
          my[idx] = F<nl_tanh>(cy[idx]);
          y[idx] = my[idx] * oy[idx];
        } else {
          _lstm_input.ComputeForwardScore(y[idx - 1], x[idx], iy[idx]);
          _lstm_forget.ComputeForwardScore(y[idx - 1], x[idx], fy[idx]);
          _lstm_cell.ComputeForwardScore(y[idx - 1], x[idx], mcy[idx]);
          cy[idx] = mcy[idx] * iy[idx] + cy[idx - 1] * fy[idx];
          _lstm_output.ComputeForwardScore(y[idx - 1], x[idx], oy[idx]);
          my[idx] = F<nl_tanh>(cy[idx]);
          y[idx] = my[idx] * oy[idx];
        }
      }
  ...
  inline void ComputeForwardScoreIncremental(Tensor<xpu, 2, dtype> x,  Tensor<xpu, 2, dtype> iy, Tensor<xpu, 2, dtype> oy, Tensor<xpu, 2, dtype> fy,
    ...
    assert(_left2right);
    _lstm_input.ComputeForwardScore(_null1, x, iy);
    _lstm_cell.ComputeForwardScore(_null1, x, mcy);
    cy = mcy * iy;
    _lstm_output.ComputeForwardScore(_null1, x, oy);
    my = F<nl_tanh>(cy);
    y = my * oy;
  }
  ...
  inline void ComputeForwardScoreIncremental(Tensor<xpu, 2, dtype> pcy, Tensor<xpu, 2, dtype> py, Tensor<xpu, 2, dtype> x, Tensor<xpu, 2, dtype> iy, Tensor<xpu, 2, dtype> oy, Tensor<xpu, 2, dtype> fy,
      Tensor<xpu, 2, dtype> mcy, Tensor<xpu, 2, dtype> cy, Tensor<xpu, 2, dtype> my, Tensor<xpu, 2, dtype> y) {
    assert(_left2right);
    _lstm_input.ComputeForwardScore(py, x, iy);
    _lstm_forget.ComputeForwardScore(py, x, fy);
    _lstm_cell.ComputeForwardScore(py, x, mcy);
    cy = mcy * iy + pcy * fy;
    _lstm_output.ComputeForwardScore(py, x, oy);
    my = F<nl_tanh>(cy);
    y = my * oy;
  }
  ...
  inline void ComputeBackwardLoss(Tensor<xpu, 3, dtype> x, Tensor<xpu, 3, dtype> iy, Tensor<xpu, 3, dtype> oy, Tensor<xpu, 3, dtype> fy,
    ...
    if (_left2right) {
      ...
      for (int idx = seq_size - 1; idx >= 0; idx--) {
        ...
        if (idx == 0) {
          _lstm_output.ComputeBackwardLoss(_null1, x[idx], oy[idx],
              loy[idx], _null1Loss, lx[idx]);
          ...
          _lstm_input.ComputeBackwardLoss(_null1, x[idx], iy[idx],
              liy[idx], _null1Loss, lx[idx]);

        } else {
          _lstm_output.ComputeBackwardLoss(y[idx - 1], x[idx], oy[idx],
              loy[idx], lFy[idx - 1], lx[idx]);
          ...
          _lstm_forget.ComputeBackwardLoss(y[idx - 1], x[idx],
              fy[idx], lfy[idx], lFy[idx - 1], lx[idx]);

          _lstm_input.ComputeBackwardLoss(y[idx - 1], x[idx],
              iy[idx], liy[idx], lFy[idx - 1], lx[idx]);
        }
      }
    } else {
      // right rnn
