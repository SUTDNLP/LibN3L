...
public:
  ...
  void writeModel(LStream &outf) {
    ...
    // int _last_update_size = _last_update.size();
    // WriteBinary(outf, _last_update_size);
    // if (_last_update_size > 0 ) {
    //   for (int idx = 0; idx < _last_update_size; idx++) {
    //     WriteBinary(outf, _last_update[idx]);
    //   }
    // }

    // cout << "_E.shape= " << _E.size(0) << "and "<< _E.size(1)<< endl;
    // cout << "_gradE.shape= " << _gradE.size(0) << "and " << _gradE.size(1)<< endl;
    // cout << "_eg2E.shape= " << _eg2E.size(0) << "and " << _eg2E.size(1) << endl;
    // cout << "_ftE.shape= " << _ftE.size(0) << "and " << _ftE.size(1) << endl;
  ...
  void loadModel(LStream &inf) {
    ...
    // int _last_update_size;
    // ReadBinary(inf, _last_update_size);
    // if (_last_update_size > 0 ) {
    //   _last_update.resize(_last_update_size);
    //   for (int idx = 0; idx < _last_update_size; idx++) {
    //     ReadBinary(inf, _last_update[idx]);
    //   } 
    // }
